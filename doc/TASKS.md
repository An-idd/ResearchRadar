...
2026-09-12 入库规则调整：用户要求只保存相关主题的论文。将关键词初筛和结构化主题确认前移至采集入库之前；无关论文不入主库，明确记录失败/预算不足，保留已确认论文的跨来源合并。此任务不包含后续 Phase，也不自动删除历史未分类论文。说明见 [ADMISSION.md](ADMISSION.md)。

2026-09-11 授权更新：用户要求使用 Neon PostgreSQL、补充文档并完成整个 MVP。本轮按 Phase 0–11 顺序实施和验收，阶段通过后继续；Phase 12–18 不在本轮范围。当前验收进度记录在 `doc/IMPLEMENTATION.md`。

2026-09-11 前端授权更新：用户要求按已列出的前端计划逐步开发，新增独立 Frontend MVP 阶段，分为工程与契约、列表、详情与证据、分析任务、主题与阅读体验、联调交付六步。实现与验收见 [FRONTEND.md](FRONTEND.md)，不改变 Phase 12–18 的待开发范围。
